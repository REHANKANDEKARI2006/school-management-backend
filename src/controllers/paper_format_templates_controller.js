...
const PaperFormatTemplatesController = {
  ...
  async getTemplate(req, res) {
    try {
      ...
      const data = await PaperFormatTemplatesModel.getTemplate({ class_name, subject, exam_type });
  ...
  async listAll(req, res) {
    try {
      const data = await PaperFormatTemplatesModel.listAll();
      res.json({ success: true, data });
    } catch (err) {
      res.status(500).json({ success: false, message: err.message });
    }
  ...
  async getById(req, res) {
    try {
      const data = await PaperFormatTemplatesModel.getById(req.params.id);
