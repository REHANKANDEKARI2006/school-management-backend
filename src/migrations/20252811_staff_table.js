...
// 3. Seed staff using user_ids from teacher/cashier/librarian/management/sportsmgr users
export async function seedStaff() {
  try {
    await db.query(`
      INSERT INTO staff (
        user_id, staff_first_name, staff_last_name, title,
      ...
      VALUES
        ...
        ((SELECT user_id FROM "user" WHERE user_name = 'teacher1'  AND institute_id = 3),
          'Ankit', 'Mathur', 'Teacher', 'teacher1@demo.edu.in',  '9100000001', 'MSc Mathematics', 2,  3, 1, 1, 1),
        ((SELECT user_id FROM "user" WHERE user_name = 'teacher2'  AND institute_id = 3),
          'Ayushi', 'Verma', 'Teacher', 'teacher2@demo.edu.in',  '9100000002', 'MA English',      1,  1, 1, 2, 2),
        ((SELECT user_id FROM "user" WHERE user_name = 'teacher3'  AND institute_id = 3),
          'Rahul', 'Singh', 'Teacher', 'teacher3@demo.edu.in',  '9100000003', 'MSc Science',      3,  5, 1, 1, 1),
        ((SELECT user_id FROM "user" WHERE user_name = 'teacher4'  AND institute_id = 3),
          'Pooja', 'Sharma', 'Teacher', 'teacher4@demo.edu.in',  '9100000004', 'MA Hindi',        1,  2, 1, 2, 2),
        ((SELECT user_id FROM "user" WHERE user_name = 'teacher5'  AND institute_id = 3),
          'Mohit', 'Kumar', 'Teacher', 'teacher5@demo.edu.in',  '9100000005', 'MSc Physics',     3,  6, 1, 1, 1),
        ((SELECT user_id FROM "user" WHERE user_name = 'teacher6'  AND institute_id = 3),
          'Simran','Patel','Teacher','teacher6@demo.edu.in',    '9100000006', 'MSc Comp. Sci.',  8,  8, 1, 2, 2),
        ((SELECT user_id FROM "user" WHERE user_name = 'teacher7'  AND institute_id = 3),
          'Ramesh','Yadav','Teacher','teacher7@demo.edu.in',    '9100000007', 'MSc Biology',     3,  7, 1, 1, 1),
        ((SELECT user_id FROM "user" WHERE user_name = 'teacher8'  AND institute_id = 3),
          'Priya', 'Jain',  'Teacher','teacher8@demo.edu.in',   '9100000008', 'MA Geography',    6, 10, 1, 2, 2),
        ((SELECT user_id FROM "user" WHERE user_name = 'teacher9'  AND institute_id = 3),
          'Vinay', 'Nair',  'Teacher','teacher9@demo.edu.in',   '9100000009', 'MA History',      5,  9, 1, 1, 1),
        ((SELECT user_id FROM "user" WHERE user_name = 'teacher10' AND institute_id = 3),
          'Deepa', 'Gupta', 'Teacher','teacher10@demo.edu.in',  '9100000010', 'MA Social Sci.', 4,  4, 1, 2, 2),

        -- Cashiers
        ((SELECT user_id FROM "user" WHERE user_name = 'cashier1' AND institute_id = 3),
          'Alok',  'Shah',  'Cashier','cashier1@demo.edu.in',   '9200000001', 'BCom Finance',    9, NULL, 1, 1, 1),
        ((SELECT user_id FROM "user" WHERE user_name = 'cashier2' AND institute_id = 3),
          'Kavita','Joshi', 'Cashier','cashier2@demo.edu.in',   '9200000002', 'BCom Accounts',   9, NULL, 1, 2, 2),

        -- Librarian
        ((SELECT user_id FROM "user" WHERE user_name = 'librarian' AND institute_id = 3),
          'Meena','Kaur','Librarian','librarian@demo.edu.in',   '9300000001', 'MLISc',          10, NULL, 1, 2, 2),

        -- Management
        ((SELECT user_id FROM "user" WHERE user_name = 'management' AND institute_id = 3),
          'Sanjay','Mehta','Management','management@demo.edu.in','9400000001','MBA',           NULL, NULL, 1, 1, 1),

        -- Sports Manager
        ((SELECT user_id FROM "user" WHERE user_name = 'sportsmgr' AND institute_id = 3),
          'Anil','Raj','Sports Manager','sportsmgr@demo.edu.in','9500000001','BPEd',           7, NULL, 1, 1, 1)
    `);
